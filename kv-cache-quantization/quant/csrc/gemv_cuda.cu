...
// based on KIVI's cuda code https://github.com/jy-yuan/KIVI
// added bmm with rope
...
__global__ void bgemv2_kernel_outer_dim_cos(
  ...
  const int IC, const int OC, const int group_size, const int nh, const int nh_kv){
    ...
    const int ratio = nh / nh_kv;
    int _batch_idx = batch_idx / ratio;
    int rope_batch_idx = batch_idx / nh;  // Get original batch index for cos/sin
    const uint32_t*  weight = _weight + _batch_idx * OC * IC / pack_factor;
    const half* scaling_factors = _scale + _batch_idx * OC * IC / group_size;
    const half* zeros = _zeros + _batch_idx * OC * IC / group_size;
    const half* cosw = _cosB + rope_batch_idx * OC * IC;
    const int TILE_DIM = 128;
    const int num = 0xFF >> (8-bit);
    // 1float4 == 8 half number
    float psum[pack_factor]{};
    for (int k=0; k < (ICR + TILE_DIM - 1) / TILE_DIM; k++){
      ...
      for (int ic_0 = 0; ic_0 < 4; ic_0++){
        ...
        for (int ic_1 = 0; ic_1 < pack_factor; ic_1++){
          int oc_idx = oc_start_idx + ic_1;
          if (oc_idx < OC){
            float cur_cos = *(cosw + oc_idx * ICR + ic_0 + k * TILE_DIM + threadIdx.x*4);
            float cur_single_weight_fp = (float)(cur_packed_weight & num);
            float dequantized_weight = cur_scale * cur_single_weight_fp + cur_zero;
            // if(blockIdx.x == 0 && blockIdx.y == 0 && threadIdx.x == 0 && threadIdx.y == 0 && k == 1) printf("%d %d %d %f %f %f %f %f\n", k, ic_0, ic_1, dequantized_weight, cur_single_weight_fp, cur_scale, cur_zero, cur_inp);
            cur_packed_weight = cur_packed_weight >> bit;
            dequantized_weight = dequantized_weight * cur_cos;
            psum[ic_1] += dequantized_weight * cur_inp;
          }
        }
      }
...
__global__ void bgemv4_kernel_outer_dim_cos(
  const half* _inputs, const uint32_t* _weight, const half* _zeros, const half* _scale, 
  const half* _cosB, half* _outputs, 
  const int IC, const int OC, const int group_size, const int nh, const int nh_kv){
    // const int group_size = 64;
    ...
    const int nh_kv)
{
    ...
    auto zeros = reinterpret_cast<half*>(_zeros.data_ptr<at::Half>());
    auto scaling_factors = reinterpret_cast<half*>(_scaling_factors.data_ptr<at::Half>());
    auto cosB = reinterpret_cast<half*>(_cosB.data_ptr<at::Half>());
    // auto out_in_map = _out_in_map.data_ptr<int>();
    auto options =
    torch::TensorOptions().dtype(_in_feats.dtype()).device(_in_feats.device());
    // kernel is [OC, IC]
    ...
    dim3 num_threads(32, 4);
    if (bit == 4){
      bgemv4_kernel_outer_dim_cos<<<num_blocks, num_threads>>>(
        // pointers
        in_feats, kernel, zeros, scaling_factors, cosB, out_feats,
        // constants
        num_in_channels, num_out_channels, group_size, nh, nh_kv
      );
    }
      ...
      // note: in this case, pack factor == 16
      bgemv2_kernel_outer_dim_cos<<<num_blocks, num_threads>>>(
        // pointers
        in_feats, kernel, zeros, scaling_factors, cosB, out_feats,
        // constants
        num_in_channels, num_out_channels, group_size, nh, nh_kv
...
__global__ void bgemv2_kernel_outer_dim_rope(
  const half* _inputs, const uint32_t* _weight, const half* _zeros, const half* _scale, 
  const half* _cosB, const half* _sinB, half* _outputs, 
  const int IC, const int OC, const int group_size, const int nh, const int nh_kv){
    // IC == K, OC == N
    const int bit = 2;
    const int pack_factor = 16;
    ...
    const half* zeros = _zeros + _batch_idx * OC * IC / group_size;
    const half* cosw = _cosB + rope_batch_idx * OC * IC;
    const half* sinw = _sinB + rope_batch_idx * OC * IC;
    int max_cos_offset = OC * ICR;
    const int TILE_DIM = 128;
    const int num = 0xFF >> (8-bit);
    // 1float4 == 8 half number
    float psum[pack_factor]{};
    float dequantized_weight_x1[pack_factor]{};
    float input_x1[pack_factor]{};
    float sin_x1[pack_factor]{};
    for (int k=0; k < (ICR + TILE_DIM - 1) / TILE_DIM; k++){
      uint32_t qw[4]{};
      half cscale[4]{};
      half czero[4]{};
      ...
      for (int ic_0 = 0; ic_0 < 4; ic_0++){
        ...
        for (int ic_1 = 0; ic_1 < pack_factor; ic_1++){
          ...
          if (oc_idx < OC){
            float cur_cos = *(cosw + oc_idx * ICR + ic_0 + k * TILE_DIM + threadIdx.x*4);
            float cur_sin = *(sinw + oc_idx * ICR + ic_0 + k * TILE_DIM + threadIdx.x*4);
            float cur_single_weight_fp = (float)(cur_packed_weight & num);
            float dequantized_weight = cur_scale * cur_single_weight_fp + cur_zero;
            if (ic_0 % 2 == 0){
              dequantized_weight_x1[ic_1] = dequantized_weight;
              input_x1[ic_1] = cur_inp;
              sin_x1[ic_1] = cur_sin;
              psum[ic_1] += dequantized_weight * cur_inp * cur_cos;
            }
            else{
              psum[ic_1] -= dequantized_weight * sin_x1[ic_1] * input_x1[ic_1];
              psum[ic_1] += (dequantized_weight * cur_cos + dequantized_weight_x1[ic_1] * cur_sin) * cur_inp;
            }
            cur_packed_weight = cur_packed_weight >> bit;
            // dequantized_weight = dequantized_weight * cur_cos;
            // psum[ic_1] += dequantized_weight * cur_inp;
...
__global__ void bgemv4_kernel_outer_dim_rope(
  const half* _inputs, const uint32_t* _weight, const half* _zeros, const half* _scale, 
  const half* _cosB, const half* _sinB, half* _outputs, 
  const int IC, const int OC, const int group_size, const int nh, const int nh_kv){
    // IC == K, OC == N
...
torch::Tensor gemv_forward_cuda_outer_dim_rope(
    ...
    const int nh_kv)
{
    int BS = _in_feats.size(0); // B * nh
    int num_in_feats = _in_feats.size(1); // M
    int num_in_channels = _in_feats.size(2); // K
    int num_out_channels = _zeros.size(1) * group_size; // N
    // int kernel_volume = _out_in_map.size(1);
    auto in_feats = reinterpret_cast<half*>(_in_feats.data_ptr<at::Half>());
    auto kernel = reinterpret_cast<uint32_t*>(_kernel.data_ptr<int>());
    auto zeros = reinterpret_cast<half*>(_zeros.data_ptr<at::Half>());
    auto scaling_factors = reinterpret_cast<half*>(_scaling_factors.data_ptr<at::Half>());
    auto cosB = reinterpret_cast<half*>(_cosB.data_ptr<at::Half>());
    auto sinB = reinterpret_cast<half*>(_sinB.data_ptr<at::Half>());
    // auto out_in_map = _out_in_map.data_ptr<int>();
    auto options =
    torch::TensorOptions().dtype(_in_feats.dtype()).device(_in_feats.device());
    // //kernel is [OC, IC]
    at::Tensor _out_feats = torch::empty({BS, num_in_feats, num_out_channels}, options); // [B * nh, M, N]
    int num_out_feats = _out_feats.size(-2); // M
    auto out_feats = reinterpret_cast<half*>(_out_feats.data_ptr<at::Half>());
    int pack_factor = 32 / bit;
    dim3 num_blocks(BS, (num_out_channels / pack_factor + 3) / 4, num_out_feats); // [B * nh, N / pack_factor / 4, M]
    dim3 num_threads(32, 4);
    if (bit == 4){
      bgemv4_kernel_outer_dim_rope<<<num_blocks, num_threads>>>(
        // pointers
        in_feats, kernel, zeros, scaling_factors, cosB, sinB, out_feats,
        // constants
        num_in_channels, num_out_channels, group_size, nh, nh_kv
      );
    }
      ...
      // note: in this case, pack factor == 16
      bgemv2_kernel_outer_dim_rope<<<num_blocks, num_threads>>>(
        // pointers
        in_feats, kernel, zeros, scaling_factors, cosB, sinB, out_feats,
        // constants
        num_in_channels, num_out_channels, group_size, nh, nh_kv
