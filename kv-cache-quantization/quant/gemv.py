...
def test_bgemv_outer_correct_mha():
	...
	for BIT in [2, 4]:
		...
		output = squat_gemv.gemv_forward_cuda_outer_dim(inp, qweight, scale, mn, BIT, GS, nh, False)
...
def test_bgemv_outer_correct_mqa():
	...
	for BIT in [2, 4]:
		...
		scale = scale.transpose(1, 2).contiguous()
		mn = mn.transpose(1, 2).contiguous()
		output = squat_gemv.gemv_forward_cuda_outer_dim(inp, qweight, scale, mn, BIT, GS, nh, True)
		deq_w = dequant_weight_outer(weight.transpose(1, 2), 
							   scale.transpose(1, 2), 
							   mn.transpose(1, 2), GS)
...
def test_gemv_correct():
	...
	for BIT in [4]:
		...
		output = squat_gemv.gemv_forward_cuda(inp, qweight, scale, mn, BIT, GS)
...
def test_gemv_speed():
	...
	stmt = "squat_gemv.gemv_forward_cuda(inp, qweight, scale, mn, BIT, GS)"
...
def test_bgemv_outer_speed():
	...
	for BIT in [2]:
		...
		t_ref = py_benchmark(stmt, {**globals(), **locals()}, min_repeat_second=1,
										setup="torch.cuda.synchronize()", finish="torch.cuda.synchronize()")
		stmt = "squat_gemv.gemv_forward_cuda_outer_dim(inp, qweight, scale, mn, BIT, GS)"
		t_our = py_benchmark(stmt, {**globals(), **locals()}, min_repeat_second=1,
										setup="torch.cuda.synchronize()", finish="torch.cuda.synchronize()")
