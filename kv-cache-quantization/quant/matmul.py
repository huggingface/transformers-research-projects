...
def interleave_tensor(x, dim=2):
    n = x.size(dim) // 2
    x1, x2 = torch.split(x, n, dim=dim)
...
def cuda_bmm_fA_qB_outer(group_size: int, 
	...
	assert bits in [2, 4]
	assert nh % nh_kv == 0
	c = squat_gemv.gemv_forward_cuda_outer_dim(fA, qB, scales, zeros, bits, group_size, nh, nh_kv)
	c = c.view(B, nh, c.shape[-2], c.shape[-1])
	return c


# Added based on KIVI's code
def cuda_bmm_fA_qB_outer_cos_sin(group_size: int, 
				fA: torch.FloatTensor, 
				qB: torch.IntTensor, 
				scales: torch.FloatTensor, 
				zeros: torch.FloatTensor,
				cosB: torch.FloatTensor,
				sinB: torch.FloatTensor,
				bits: int) -> torch.FloatTensor:
	""" bmm with rope, using two cos calls
	"""
	assert len(fA.shape) == 4 and len(qB.shape) == 4
	B, nh, M, K = fA.shape 
	nh_kv =  qB.shape[1]
	...
	fA21 = torch.cat([fA[:,:,K//2:], fA[:,:,:K//2]], dim=-1)
	sinB2m1 = torch.cat([sinB[:,:,K//2:], -sinB[:,:,:K//2]], dim=-1)
	c = squat_gemv.gemv_forward_cuda_outer_dim_cos(fA, qB, scales, zeros, cosB, bits, group_size, nh, nh_kv) + \
	    squat_gemv.gemv_forward_cuda_outer_dim_cos(fA21, qB, scales, zeros, sinB2m1, bits, group_size, nh, nh_kv)
	c = c.view(B, nh, c.shape[-2], c.shape[-1])
	return c


def cuda_bmm_fA_qB_outer_rope(group_size: int, 
				fA: torch.FloatTensor, 
				qB: torch.IntTensor, 
				scales: torch.FloatTensor, 
				zeros: torch.FloatTensor,
				...
				sinB: torch.FloatTensor,
				bits: int) -> torch.FloatTensor:
	""" bmm with rope, using one rope call, this is a little bit slower than two cos calls
	"""
	assert len(fA.shape) == 4 and len(qB.shape) == 4
	B, nh, M, K = fA.shape 
	nh_kv =  qB.shape[1]
	...
	fA_interleave = interleave_tensor(fA, dim=2)
	qB_interleave = interleave_tensor(qB, dim=2)
	cosB_interleave = interleave_tensor(cosB, dim=2)
	sinB_interleave = interleave_tensor(sinB, dim=2)
	scales_interleave = interleave_tensor(scales, dim=2)
	zeros_interleave = interleave_tensor(zeros, dim=2)

	c = squat_gemv.gemv_forward_cuda_outer_dim_rope(
		fA_interleave, qB_interleave, scales_interleave, zeros_interleave, 
		cosB_interleave, sinB_interleave, bits, group_size, nh, nh_kv)
	c = c.view(B, nh, c.shape[-2], c.shape[-1])
	return c
