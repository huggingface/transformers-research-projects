...
from sympy import N, latex, simplify
...
def extract(string, dataset="math"):
    return strip_string(extract_answer(string, dataset))


def weighted_best_of_n(responses, rewards):
    """
    Implements Weighted Best-of-N to select the response with the highest total reward.

    Args:
        responses (list): List of candidate responses.
        rewards (list): List of reward scores corresponding to each response.

    Returns:
        str: The response with the highest weighted total reward.
    """
    # Dictionary to store aggregated rewards for each unique response
    weighted_rewards = defaultdict(float)

    # Aggregate rewards for each unique response
    for response, reward in zip(responses, rewards):
        weighted_rewards[response] += reward

    # Find the response with the maximum total reward
    best_response = max(weighted_rewards, key=weighted_rewards.get)

    return best_response


def get_pass_rate(responses, answer, dataset="math"):
    is_correct_list = []
    for response in responses:
        is_correct_list.append(
            math_equal(
                memoized_canonical_form(answer),
                memoized_canonical_form(extract(response, dataset=dataset)),
            )
        )
    return any(is_correct_list)
